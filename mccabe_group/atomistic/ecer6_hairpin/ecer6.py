import mbuild as mb
class ecer6(mb.Compound):
    def __init__(self, use_atom_name=True):
        super(ecer6,self).__init__()
        if use_atom_name:
            mb.load('ecer6_new.mol2', compound=self, relative_to_module=self.__module__)
        else:
            mb.load('ecer6.mol2', compound=self, relative_to_module=self.__module__)
        self.children[0].charge = -0.53
        self.children[1].charge = 0.26
        self.children[2].charge = 0.58
        self.children[3].charge = -0.53
        self.children[4].charge = 0.16
        self.children[5].charge = 0.09
        self.children[6].charge = -0.56
        self.children[7].charge = -0.18
        self.children[8].charge = 0.09
        self.children[9].charge = 0.09
        self.children[10].charge = -0.18
        self.children[11].charge = 0.09
        self.children[12].charge = 0.09
        self.children[13].charge = -0.18
        self.children[14].charge = 0.09
        self.children[15].charge = 0.09
        self.children[16].charge = -0.18
        self.children[17].charge = 0.09
        self.children[18].charge = 0.09
        self.children[19].charge = -0.18
        self.children[20].charge = 0.09
        self.children[21].charge = 0.09
        self.children[22].charge = -0.18
        self.children[23].charge = 0.09
        self.children[24].charge = 0.09
        self.children[25].charge = -0.18
        self.children[26].charge = 0.09
        self.children[27].charge = 0.09
        self.children[28].charge = -0.18
        self.children[29].charge = 0.09
        self.children[30].charge = 0.09
        self.children[31].charge = -0.18
        self.children[32].charge = 0.09
        self.children[33].charge = 0.09
        self.children[34].charge = -0.18
        self.children[35].charge = 0.09
        self.children[36].charge = 0.09
        self.children[37].charge = -0.18
        self.children[38].charge = 0.09
        self.children[39].charge = 0.09
        self.children[40].charge = -0.18
        self.children[41].charge = 0.09
        self.children[42].charge = 0.09
        self.children[43].charge = -0.18
        self.children[44].charge = 0.09
        self.children[45].charge = 0.09
        self.children[46].charge = -0.3
        self.children[47].charge = 0.1
        self.children[48].charge = 0.1
        self.children[49].charge = 0.1
        self.children[50].charge = 0.03
        self.children[51].charge = 0.12
        self.children[52].charge = 0.06
        self.children[53].charge = 0.105
        self.children[54].charge = 0.105
        self.children[55].charge = -0.56
        self.children[56].charge = 0.34
        self.children[57].charge = 0.15
        self.children[58].charge = 0.09
        self.children[59].charge = -0.56
        self.children[60].charge = 0.34
        self.children[61].charge = 0.16
        self.children[62].charge = 0.09
        self.children[63].charge = -0.56
        self.children[64].charge = 0.31
        self.children[65].charge = -0.2
        self.children[66].charge = 0.1
        self.children[67].charge = 0.1
        self.children[68].charge = -0.18
        self.children[69].charge = 0.09
        self.children[70].charge = 0.09
        self.children[71].charge = -0.18
        self.children[72].charge = 0.09
        self.children[73].charge = 0.09
        self.children[74].charge = -0.18
        self.children[75].charge = 0.09
        self.children[76].charge = 0.09
        self.children[77].charge = -0.18
        self.children[78].charge = 0.09
        self.children[79].charge = 0.09
        self.children[80].charge = -0.18
        self.children[81].charge = 0.09
        self.children[82].charge = 0.09
        self.children[83].charge = -0.18
        self.children[84].charge = 0.09
        self.children[85].charge = 0.09
        self.children[86].charge = -0.18
        self.children[87].charge = 0.09
        self.children[88].charge = 0.09
        self.children[89].charge = -0.18
        self.children[90].charge = 0.09
        self.children[91].charge = 0.09
        self.children[92].charge = -0.18
        self.children[93].charge = 0.09
        self.children[94].charge = 0.09
        self.children[95].charge = -0.18
        self.children[96].charge = 0.09
        self.children[97].charge = 0.09
        self.children[98].charge = -0.18
        self.children[99].charge = 0.09
        self.children[100].charge = 0.09
        self.children[101].charge = -0.18
        self.children[102].charge = 0.09
        self.children[103].charge = 0.09
        self.children[104].charge = -0.3
        self.children[105].charge = 0.1
        self.children[106].charge = 0.1
        self.children[107].charge = 0.1
        self.children[108].charge = 0.31
